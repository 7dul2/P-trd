function anim(){
    ...
    tl.to("#left", {duration: 0.4, opacity: 1, x: 0});
    // 在第一个动画开始后的0.1秒后开始第二个动画
    tl.to("#right", {duration: 0.4, opacity: 1,x :0}, "-=0.1");
    ...
    tl.to("#animation_loading", {duration: 0.4, opacity: 1}, "-=0.1");
    // 将 #load 元素透明度渐变为0，同时缩小至0，延迟1.5秒后执行
    tl.to("#load", {duration: 0.25, opacity: 0, scale: 1.5, delay: 0.75});
